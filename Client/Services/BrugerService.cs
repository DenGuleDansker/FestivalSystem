...
namespace Client.Services
{
    public class BrugerService : IBrugerService // Klassen arver fra IBrugerService interfacet 
    {
        ...
        {
            var brugerlist = await http.GetFromJsonAsync<Bruger[]>("http://api.chilinh.dk/api/brugere");
            return brugerlist;
        }

        public async Task<Bruger> GetBruger(int id) // Henter information på den enkelte bruger, baseret på deres id
        {
            return await http.GetFromJsonAsync<Bruger>($"http://api.chilinh.dk/api/brugere/bruger/{id}");
        }

        public async Task Add(Bruger bruger) // Tilføjer en bruger 
        {
            await http.PostAsJsonAsync<Bruger>("http://api.chilinh.dk/api/brugere", bruger); // Sender en POST request med booking som JSON payload til API'en
        ...
        {
            await http.PostAsJsonAsync<Bruger>("http://api.chilinh.dk/api/brugere/update", bruger);
        ...
        {
            var result = await http.GetFromJsonAsync<Bruger>("http://api.chilinh.dk/api/brugere/" + email);

            return result;
        }
        public void DeleteBruger(int ID) // Sletter en bruger baseret på et specifikt ID
        {
            http.DeleteFromJsonAsync<Bruger>($"http://api.chilinh.dk/api/brugere/{ID}");
